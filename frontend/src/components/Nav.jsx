...
function Nav() {
    ...
    return (
        <div className='w-full h-[80px] flex items-center justify-between md:justify-center gap-[30px] px-[20px] fixed top-0 z-[9999] bg-[#fff9f6] outline-visible'>
            ...
            {/* app logo */}
            <h1 className='text-3xl font-bold mb-2 text-[#ff4d2d]'>KriShi</h1>
            ...
            <div className='flex items-center gap-6'>
                ...
                {userData.role == "user" &&
                    (showSearch ? <RxCross2 size={25} className='text-[#ff4d2d] md:hidden' onClick={() => {
                        setShowSearch(false)
                ...
                {userData.role == "owner" ?
                    <>
                        ...
                        <div className='md:hidden flex items-center gap-2 cursor-pointer relative px-3 py-1 rounded-lg bg-[#ff4d2d]/10 text-[#ff4d2d] font-medium' title='My Orders' onClick={() => navigate("/my-orders")}>
                            <LuReceipt size={20} />
                            <span className='absolute -right-2 -top-2 text-xs font-bold text-white bg-[#ff4d2d] rounded-full px-[6px] py-[1px]'>0</span>
                        </div>
                    </> : (
                ...
                {/* Profile logo */}
                <div className='w-[40px] h-[40px] rounded-full flex items-center justify-center bg-[#ff4d2d] text-white text-[18px] shadow-xl font-semibold cursor-pointer' onClick={() => setShowInfo(val => !val)}>
                    {userData?.fullName?.slice(0, 1)}
                </div>

                {/* Profile options */}
                {showInfo &&
                    <div className={`fixed top-[80px] right-[10px] ${userData.role == "deliveryBoy" ? "md:right-[10%] lg:right-[30%]" : "md:right-[10%] lg:right-[25%]"} w-[180px] bg-white shadow-2xl rounded-xl p-[20px] flex flex-col gap-[10px] z-[9999]`}>
